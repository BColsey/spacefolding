...
## Gate verification + a self-corpus-fixture caveat (owner decision flagged)
...
This is a **pre-existing gate fragility** Phase 6 surfaced: the self-corpus
reference drifts as the repo grows, and a large-enough legitimate source addition
can exceed the 0.03 margin on the file-level BM25F comparator even with the product
unchanged. Resolving it is an **owner decision** (the discipline forbids lowering a
threshold / re-pinning the gate baseline unilaterally to force green):
- **(recommended) Freeze the gate's self-corpus fixture** to a stable committed
  snapshot (or a pinned git ref / a curated `src/` subset, excluding the benchmark
  harness like `tests/` already is), so the reference no longer drifts with repo
  growth. Requires a one-time re-pin against the frozen fixture.
- **Refresh `deterministic-baseline.json` at clean HEAD** — record the current
  corpus snapshot (structural improves 0.873→0.886; only the bm25 comparator
  reference moves down, reflecting the larger corpus). Simplest, but re-pins a
  comparator reference, so it needs an explicit owner sign-off to not read as
  bar-lowering.

No baseline change is committed in this work pending that decision.
