...
// Group by label → {original?, ablated?}
const byLabel = new Map<string, { original?: Run; ablated?: Run }>();
for (const run of runs) {
  let entry = byLabel.get(run.label);
  if (!entry) { entry = {}; byLabel.set(run.label, entry); }
...
for (const [label, { original }] of byLabel) {
  ...
  out.push(`| ${label} | ${fmt(sT)} | ${fmt(gCtx)} | ${fmt(gWhole)} | ${fmt(fT)} | ${fmt(bT)} | ${ciStr} | ${ci?.n ?? '—'} |`);
  if (ci?.excludesZero && ci.mean < 0) crossover.push(`${label}: structural wins (Δ ${Math.round(ci.mean)} [${Math.round(ci.low)}..${Math.round(ci.high)}], n=${ci.n})`);
  else if (ci?.excludesZero && ci.mean > 0) crossover.push(`${label}: GREP wins tokens-to-first-correct-file (Δ +${Math.round(ci.mean)}, n=${ci.n})`);
}
...
for (const [label, { original }] of byLabel) {
  ...
  out.push(`| ${label} | ${S.averages.recallAt10.toFixed(3)}/${S.averages.hitsAt1.toFixed(3)} | ${G.averages.recallAt10.toFixed(3)}/${G.averages.hitsAt1.toFixed(3)}/${gR8?.toFixed(3) ?? '—'} | ${F?.averages.recallAt10.toFixed(3) ?? '—'} | ${B?.averages.recallAt10.toFixed(3) ?? '—'} |`);
}
...
out.push('|---|---|---|');
for (const [label, { original, ablated }] of byLabel) {
  ...
  out.push(`| ${label} | ${So.averages.hitsAt1.toFixed(3)}→${Sa.averages.hitsAt1.toFixed(3)} | ${Go.averages.hitsAt1.toFixed(3)}→${Ga.averages.hitsAt1.toFixed(3)} |`);
}
